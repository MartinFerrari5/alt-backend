  ...
  try {
    ...
    for (let user of users) {
      ...
      if (rows.length === 0) {
        ...
        `;
        await conn.query(userInsertQuery, [user.name, user.email, user.password, user.isAdmin]);
    ...
    `;
    await conn.query(productInsertQuery, [data.map(product => [
      product.name, product.slug, product.category, product.image, product.price, product.countInStock,
      product.brand, product.rating, product.numReviews, product.description
    ])]);
