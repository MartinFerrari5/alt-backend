...
function verifyEmailMiddleware(requested ) {
  return async (req, res, next) => {
    try {
      ...
      const query = requested ? `SELECT * FROM ${users_table} WHERE email = ?;`   : `SELECT * FROM ${emails_table} WHERE email = ?;`;
      const [user] = await connection.query(query, [email]);
      if (!user[0] && requested) {
        return res.status(404).json({ message: "Email no encontrado" });
      }
      else if (!user[0] && !requested) {
        return res.status(400).json({ message: "Email no habilitado a crear usuario" });
      }
